...
categorical_order = [
    "L22", "L23", "L39", "L40", "L41", "L42", "L43", "L44", "L45", "L46",
    "L47", "L48", "L49", "L50", "L51", "L52", "L53", "L54", "L55", "L56",
    "L57", "L58", "L59", "L60", "L61", "L62", "L64", "L125", "L126", "L127", "L128"
]
...
def _safe_int(val):
    """Parse the first integer from a string like '835' or '835-836'."""
    try:
    ...
    with open(vcf_path) as f:
        for line in f:
            ...
            if line.startswith("#CHROM"):
                ...
                samples = [col.split('_')[0] for col in cols[9:]]
                sorted_samples = sorted(samples, key=lambda x: int(''.join(filter(str.isdigit, x))))
                continue
            ...
            cols = line.strip().split("\t")
            missing_variant = sum(1 for sample in cols[9:] if sample.startswith('./.'))
            carriers = len(sorted_samples) - missing_variant
            ...
            # ── Parse protein position (numeric, first residue for ranges) ──
            prot_pos_raw = csq.get("Protein_position", "")
            prot_pos = _safe_int(prot_pos_raw)
    ...
    for df_row in df.itertuples():
        ...
        for i, sname in enumerate(sorted_samples):
            ...
            if "." in alleles:
                ...
                gt_class = "Homozygous Reference"
            elif len(set(alleles)) == 1:
                gt_class = "Homozygous Alternate"
            else:
            ...
            rows.append({
                ...
                "HGVSp":             df_row.HGVSp,
                "Protein_position":  df_row.Protein_position,   # ← NEW
