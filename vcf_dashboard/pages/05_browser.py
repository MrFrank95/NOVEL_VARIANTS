...
st.caption(
    ...
    "**GT**: Genotype (1-2/0 = 0/1-2 = Heterozygous, 1/1 = Homozygous Alternate, 0/0 = Homozygous Reference) · "
    "**DP**: Read Depth · "
    "**GQ**: Genotype Quality (≥20 reliable) ·"
)
...
if variant_options:
    ...
    var_sample_data = sdf[(sdf["CHROM"]==row["CHROM"]) & (sdf["POS"]==row["POS"])].iloc[:len(samples)]
    if len(var_sample_data):
        gt_class_colors = {"Homozygous Reference":"#d5e8d4",
                           "Heterozygous":"#ffe6cc",
                           "Homozygous Alternate":"#f8cecc",
                           "Missing":"#f0f0f0"}
        var_sample_data = var_sample_data[["sample","GT","GT_CLASS","DP","GQ"]].copy()
        for col in ['DP', 'GQ']:
            var_sample_data[col] = pd.to_numeric(var_sample_data[col], errors='coerce').astype(pd.Int64Dtype())
        var_sample_data.reset_index(drop=True, inplace=True)
        var_sample_data.index += 1
        st.dataframe(
            var_sample_data.style.map(
