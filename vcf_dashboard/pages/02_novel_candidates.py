...
if len(noncan_cand) == 0:
    ...
    for _, row in noncan_cand.sort_values(["IMPACT","VQSLOD"],
        ...
        with st.expander(
            ...
            # Match on CHROM + POS + REF + ALT with explicit type coercion to
            # avoid silent mismatches when POS is int in one df and str in another,
            # or when multiple alleles share the same position.
            carriers = sample_df[
                (sample_df["CHROM"].astype(str) == str(row["CHROM"])) &
                (sample_df["POS"].astype(str)   == str(row["POS"])) &
                (sample_df["REF"].astype(str)   == str(row["REF"])) &
                (sample_df["ALT"].astype(str)   == str(row["ALT"])) &
                (sample_df["GT_CLASS"].isin(["het","hom_alt"]))
            ][["sample","GT","GT_CLASS","DP","GQ"]].copy()

            if len(carriers):
                st.markdown(
                    f"**Carriers: {len(carriers)}/{len(samples)} samples** "
                    f"— GT: genotype (0/1 = het, 1/1 = hom alt) · "
                    f"DP: read depth · GQ: genotype quality (≥20 reliable)"
                )
                st.dataframe(carriers.reset_index(drop=True), width='stretch')
            else:
